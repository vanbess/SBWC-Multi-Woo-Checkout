...
if (!class_exists('woocommerce')) :
    ...
    class MWC {
        ...
        private static function init_hooks() {
            ...
            // script
            // add_action('wp_enqueue_scripts', array('MWC', 'load_resources'), PHP_INT_MAX);
            ...
            add_action('woocommerce_cart_calculate_fees', array('MWC', 'mwc_apply_cart_discounts'), PHP_INT_MAX);
        ...
        public static function mwc_apply_cart_discounts($cart) {
            ...
            file_put_contents(MWC_PLUGIN_DIR . 'bundle_data.log', print_r($bundle_data, true));
            file_put_contents(MWC_PLUGIN_DIR . 'cart_data.log', print_r($cart, true));
            ...
            foreach ($cart->get_cart() as $cart_item) :
                ...
                if (isset($cart_item['mwc_off_discount'])) :
                    ...
                    // retrieve discount
                    $disc_perc = (int)$cart_item['mwc_off_discount'];
                    ...
                    // discount fee name
                    $fee_name = sprintf(__('Buy %s + Get %d&#37; Off', 'woocommerce'), $item_qty, $disc_perc);
            ...
            file_put_contents(MWC_PLUGIN_DIR . 'free_paid.txt', 'paid prod count: ' . $paid_prod_count . PHP_EOL, FILE_APPEND);
            file_put_contents(MWC_PLUGIN_DIR . 'free_paid.txt', 'free prod count: ' . $free_prod_count . PHP_EOL, FILE_APPEND);
            file_put_contents(MWC_PLUGIN_DIR . 'free_paid.txt', 'free prod value: ' . $free_discount . PHP_EOL, FILE_APPEND);

            // ***************************************
            // Do cart fee calc for discount products
            // ***************************************
            if ($disc_perc !== 0) :
        ...
        public static function add_calculate_bundle_fee($cart) {
            ...
            foreach ($cart->get_cart() as $cart_item) :
                ...
                if (isset($cart_item['multi_woo_checkout']) && isset($cart_item['product_id']) && isset($cart_item['quantity']) && isset($cart_item['bundle_id'])) :
                    ...
                    // get subtotal cart
                    $subtotal += (float)$cart_item['data']->get_price() * $cart_item['quantity'];
            ...
            // bail if no bundle discount data
            if ($bundle_discount_data === false) :
                return;
            endif;

            // decode discount data if needed
            $bundle_discount_data = is_array($bundle_discount_data) ? $bundle_discount_data : json_decode($bundle_discount_data, true);

            // apply discount FREE
            if ($bundle_discount_data['selValue'] == 'free') :

                if ($cart_qty >= ($bundle_discount_data['selValue_free']['quantity'] + $bundle_discount_data['selValue_free_prod']['quantity'])) :

                    $free_prod = $bundle_discount_data['selValue_free_prod'];

                    if (wc_get_product($free_prod['post']['id'])) :

                        // get custom price mwc product
                        if (isset($bundle_discount_data['custom_price'][end($cart_prod_post_id)][$current_curr])) :
                            $free_price = $bundle_discount_data['custom_price'][end($cart_prod_post_id)][$current_curr];
                        else :
                    ...
                    if ($discount > 0) :
                        $disc_name = sprintf(__('Buy %s + Get %d FREE', 'woocommerce'), $bundle_discount_data['selValue_free']['quantity'], $free_prod['quantity']);
            ...
            // apply discount OFF
            elseif ($bundle_discount_data['selValue'] == 'off') :

                if ($cart_qty >= $bundle_discount_data['selValue_off']['quantity']) :
                    $discount = ($subtotal * $bundle_discount_data['selValue_off']['coupon']) / 100;
                    if ($discount > 0) :
                        $disc_name = sprintf(__('Buy %s + Get %d&#37; Off', 'woocommerce'), $bundle_discount_data['selValue_off']['quantity'], $bundle_discount_data['selValue_off']['coupon']);
                        $cart->add_fee($disc_name, -$discount, true);
                    endif;
                endif;
            ...
            // apply discount Bundle products
                ...
                $bundle_data = $bundle_discount_data['selValue_bun']['post'];

                if (is_array($bundle_data) || is_object($bundle_data) && !empty($bundle_data)) :

                    $bun_tt_qty = count($bundle_data);

                    if ($cart_qty >= $bun_tt_qty && $bundle_discount_data['discount_percentage'] > 0) :
                        $discount = ($subtotal * $bundle_discount_data['discount_percentage']) / 100;
                        if ($discount > 0) :
                            $disc_name = sprintf(__('Discount bundle %d&#37;', 'woocommerce'), $bundle_discount_data['discount_percentage']);
                            $cart->add_fee($disc_name, -$discount, true);
                        endif;
                    endif;
        ...
        /**
         * add_calculate_product_addon_fee
         *
         * @param object $cart
         * @return void
         */
            ...
            foreach ($cart->get_cart() as $cart_item) :
                ...
                if (isset($cart_item['mwc_addon'])) :
                    ...
                    if ($addon_discount) :
                        ...
                        $discounted_price = $reg_price - ($addon_discount * $reg_price) / 100;
                ...
                if (isset($cart_item['multi_woo_checkout']) && isset($cart_item['bundle_id'])) :
        ...
        public static function load_resources() {
            ...
            wp_localize_script(
                ...
                [
                    ...
                    'cart_url'           => $cart_url,
                    'checkout_url'       => $checkout_url,
                    'nonce_atc_new'      => wp_create_nonce('add multiple items to cart'),
                    'nonce_checkout_new' => wp_create_nonce('checkout multiple items')
                ]
            );
        }

        /**
         * Add to cart JS is moved here now for better/easier bugfixing
         *
         * @return void
         */
                                    ...
                                    if (!isset($var_data[$valueplugify[1]]) && $product->is_type('variable')) :
                                        ...
                                        foreach ($product->get_available_variations() as $key => $value) :
                                            ...
                                            array_push($var_arr, [
                                                ...
                                                'price'      => $product['custom_price'][$value['variation_id']][$current_curr],
        ...
        public static function mwc_get_price_summary_table() {
            ...
            if ($price_list) :
                ...
                foreach ($price_list as $bundle_price) :
                    ...
                    if (isset($bundle_price['label']) && isset($bundle_price['price'])) :
